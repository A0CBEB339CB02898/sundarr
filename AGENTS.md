...
Web Console 使用统一 `/app/discover` 媒体发现模块承载目录搜索、筛选、趋势、分类、关注入口和发现型海报墙；媒体详情使用 `/app/discover/:media_subject_id`。
...
`/app/discover` 默认采用内容流模式，顶部提供目录搜索和快捷筛选，下方依次展示电影趋势、剧集趋势、分类推荐和关注更新；发起搜索或筛选后，主体切换为统一海报网格。
媒体发现搜索条件必须写入 URL query，刷新、返回和分享 URL 时可以恢复。
媒体发现页使用“趋势 / 电影 / 剧集 / 动漫 / 综艺”一级探索导航，趋势位于第一位；搜索独立位于导航右侧，筛选使用平铺标签并同步 URL。趋势分区使用 Provider 的 `trending` 操作，不得暗示为平台另行定义的 `popular` 榜；电影和剧集一级入口使用 `categories` 并默认按 `popularity` 排序。
