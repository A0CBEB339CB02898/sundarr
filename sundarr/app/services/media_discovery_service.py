...
from sundarr.app.services.media_identity_service import (
    MediaIdentityConflictError,
    media_identity_service,
)
...
class MediaDiscoveryService:
    ...
    def upsert_item(self, db: Session, provider_id: str, item: CatalogItem) -> MediaSubject:
        return media_identity_service.upsert_item(
            db,
            provider_id,
            item,
            now=self._now(),
    ...
    def summary_from_subject(
        ...
        *,
        ...
        return media_identity_service.summary_from_subject(
            db,
            subject,
    ...
    def _find_provider_external_id(
        ...
        return media_identity_service.find_provider_external_id(
            db,
            media_subject_id,
            provider_id,
            provider,
    ...
    def _refresh_flags(self, db: Session, items: list[MediaSubjectSummary]) -> None:
        media_identity_service.refresh_flags(db, items)
