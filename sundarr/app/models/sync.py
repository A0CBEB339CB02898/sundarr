...
class SyncBinding(TimestampMixin, Base):
    ...
    remote_library_id: Mapped[str] = mapped_column(ForeignKey("remote_media_libraries.id"), nullable=False)
    local_library_id: Mapped[str] = mapped_column(ForeignKey("media_libraries.id"), nullable=False)
    delete_source_after_success: Mapped[bool | None] = mapped_column()
    delete_empty_source_dirs: Mapped[bool | None] = mapped_column()


class SyncSeenFile(TimestampMixin, Base):
    __tablename__ = "sync_seen_files"

    id: Mapped[str] = mapped_column(Text, primary_key=True)
    binding_id: Mapped[str | None] = mapped_column(ForeignKey("sync_bindings.id"), index=True)
