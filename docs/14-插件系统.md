...
| `CATALOG_PROVIDER` | 搜索、热门、分类和详情等媒体目录能力 | API | 合同、Registry、Activation 和 TMDb 离线实现已完成；待真实数据验收 |
...
PluginConfig 当前响应和日志已脱敏，但 config_data 仍是明文 JSON；真实凭据验收前必须补齐静态加密。
当前默认数据库没有仓库，运行时搜索源为 0。
TMDb 已在官方外部仓库实现 Manifest v2 和离线合同，但尚未通过有效 Token 的真实数据与 Web Console 验收；豆瓣和 SeedHub 的正式 v2 验收仍待完成。
```
