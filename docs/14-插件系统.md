...
| --- | --- | --- | --- |
| `SOURCE` | 按关键词搜索具体资源及其链接候选 | API | v1 兼容与 v2 Activation 已实现；SeedHub 本地真实回归通过，待官方远程发布 |
...
```text
TMDb 已在官方外部仓库实现 Manifest v2，并于 2026-08-31 通过有效 Token 的实时测试、Core 真实 API、Web Console 海报墙/详情和降级验收。SeedHub v2 的本地真实纵向切片已通过，官方 GitHub 发布尚待完成；豆瓣目录与豆瓣想看的正式 v2 验收仍未开始。
```

Phase 10.1 已完成当前 MVP 类型共用的 v2 加载、类型专用 Registry、健康检查、候选原子切换、失败回滚和启动恢复。Phase 10.2 已完成媒体发现 Core 通用消费框架的结构性验收，没有建设面向用户的 Mock 数据链。Phase 10.3 的首个 TMDb 端到端已通过并冻结 Plugin API v2；SeedHub SOURCE v2 的本地锁定提交、真实 API、收藏、启动恢复和 Web Console 验收也已通过，当前只待官方远程发布，随后实现豆瓣目录和豆瓣想看。冻结表示现有 Manifest v2 与公共运行合同必须保持向后兼容；若确需破坏性修改，必须使用新的协议版本，而不是静默改变 `plugin_api_version = "1.0"` 的语义。
