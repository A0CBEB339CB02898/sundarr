...
| `WATCHLIST_PROVIDER` | 拉取外部想看列表项并返回增量游标 | Worker 调度，API 提供测试入口 | 合同、Registry 与 Activation 已实现；豆瓣想看真实插件待开发 |
