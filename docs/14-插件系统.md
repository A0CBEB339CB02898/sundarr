...
| `WATCHLIST_PROVIDER` | 拉取外部想看列表项并返回增量游标 | Worker 调度，API 提供手动同步入口 | 豆瓣想看已锁定 `87c6c69` 并完成真实验收 |
...
```text
TMDb 已在官方外部仓库实现 Manifest v2，并于 2026-08-31 通过有效 Token 的实时测试、Core 真实 API、Web Console 海报墙/详情和降级验收。SeedHub v2 已从官方 GitHub 仓库锁定 `a10b902`，真实纵向切片和启动恢复通过；豆瓣目录与豆瓣想看已随官方插件仓库锁定 `87c6c69`，搜索、热门、分类、详情、多 Provider、海报兼容、想看持久游标、独立启停和 Web Console 验收均通过。
```

Phase 10.1 已完成当前 MVP 类型共用的 v2 加载、类型专用 Registry、健康检查、候选原子切换、失败回滚和启动恢复。Phase 10.2 已完成媒体发现 Core 通用消费框架的结构性验收，没有建设面向用户的 Mock 数据链。Phase 10.3 的 TMDb、SeedHub、豆瓣目录和豆瓣想看真实纵向切片均已通过；官方仓库分别锁定 `a10b902` 与 `87c6c69`。冻结表示现有 Manifest v2 与公共运行合同必须保持向后兼容；若确需破坏性修改，必须使用新的协议版本，而不是静默改变 `plugin_api_version = "1.0"` 的语义。
