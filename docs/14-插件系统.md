...
本文档是 Sundarr 通用插件体系的事实来源，定义插件分类、运行边界、任务流关系和生命周期。更新时间：2026-09-21。
...
| `WATCHLIST_PROVIDER` | 拉取外部想看列表项并返回增量游标 | Worker 调度，API 提供手动同步入口 | 豆瓣想看随官方仓库锁定 `94be9a8` 并完成真实验收 |

### 2.2 MVP 发布后已排期类型

| PluginType | 稳定职责 | 主要调用进程 | 边界 |
| --- | --- | --- | --- |
| `CLOUD_SAVE_PROVIDER` | 将用户确认的资源链接服务端保存到指定网盘目录，并返回外部任务状态 | API 发起，Worker 轮询 | Phase 10.6；不直接写入本地媒体库 |
| `TRANSFER_DRIVER` | 执行一种来源到目标媒体库的搬运，并报告进度、校验和取消结果 | Worker | Phase 10.7 统一 SMB，Phase 10.8—10.9 接入网盘直链和 qBittorrent |

`NOTIFICATION` 仍是未排期扩展，不参与同步状态机的同步阻塞路径。
...
当前 `SmbWriter` 和远程媒体库扫描状态机仍是 Core 内置实现，不是外部插件。Phase 10.7 引入 `TRANSFER_DRIVER` 时，先让内置 SMB 驱动实现相同协议，再接入外部驱动，不能为了插件化提前改写已稳定主链路。
...
```text
CLOUD_PROVIDER      过于泛化，不作为顶层类型；服务端保存使用 CLOUD_SAVE_PROVIDER，直链认证连接器作为 TRANSFER_DRIVER 的依赖或内部能力。
...
```text
用户确认的资源链接
  -> AcquisitionRequest(mode=save_to_cloud)
  -> CLOUD_SAVE_PROVIDER
  -> 网盘目标目录
  -> 既有 RemoteLibraryScanner 后续发现

用户确认的直链、网盘文件或种子来源
  -> AcquisitionRequest(mode=direct_transfer)
  -> Core 依据来源协议、目标媒体库和策略选择 TRANSFER_DRIVER
  -> 在任务创建前持久化 driver_id、版本与规范化 source/target
  -> TransferTask -> Worker 调用选定 Driver
```

插件不得在任务运行中临时决定“下一步流入哪个插件类型”。选择结果由 Core 在任务创建前确定并持久化，重试必须继续使用可追踪的驱动版本，除非用户明确重新规划任务。

Phase 10.9 的 qBittorrent 驱动直接把内容落到目标本地媒体库所在文件系统，并使用同文件系统的未完成标记或原子可见策略；不要求先下载到另一块本地暂存空间，也不绕经远程媒体库。该能力不改变 BT/磁力不进入 MVP 发布门的边界。
...
```text
TMDb 已在官方外部仓库实现 Manifest v2，并于 2026-08-31 通过有效 Token 的实时测试、Core 真实 API、Web Console 海报墙/详情和降级验收。SeedHub v2 功能验收提交为 `a10b902`、当前正式锁定为 `7728684`，真实纵向切片和启动恢复通过；豆瓣目录与豆瓣想看已随官方插件仓库锁定 `94be9a8`，搜索、热门、分类、详情、多 Provider、海报兼容、想看持久游标、独立启停和 Web Console 验收均通过。
```

Phase 10.1 已完成当前 MVP 类型共用的 v2 加载、类型专用 Registry、健康检查、候选原子切换、失败回滚和启动恢复。Phase 10.2 已完成媒体发现 Core 通用消费框架的结构性验收，没有建设面向用户的 Mock 数据链。Phase 10.3 的 TMDb、SeedHub、豆瓣目录和豆瓣想看真实纵向切片均已通过；官方仓库当前规范分别锁定 `7728684` 与 `94be9a8`。冻结表示现有 Manifest v2 与公共运行合同必须保持向后兼容；若确需破坏性修改，必须使用新的协议版本，而不是静默改变 `plugin_api_version = "1.0"` 的语义。
