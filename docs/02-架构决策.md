...
当列表条目缺少 release_year 且 Provider 支持 detail 时，Web Console 按 Provider 分组调用 Core 年份补全 API。
Core 每批最多接受 12 个已持久化 MediaSubject，顺序读取详情，复用详情缓存，并把成功得到的年份写回 PostgreSQL 最小快照。
...
理由：豆瓣热门和分类公开响应不包含年份，完整补全只能额外读取条目详情。首屏同步发起数十个详情请求会显著增加延迟和触发站点保护；渐进补全可以保持页面先可用，同时通过 Core 缓存、持久化和低并发逐步消除“年份未知”。该机制属于 Core 通用目录能力，不在 Core 中硬编码豆瓣字段。
