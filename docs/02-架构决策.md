...
Phase 10.3 在官方外部插件仓库逐个实现 TMDb、SeedHub、豆瓣目录和豆瓣想看等真实插件。插件开发期间必须以真实数据持续回归 Core；TMDb 端到端已于 2026-08-31 通过并冻结 Plugin API v2，SeedHub SOURCE v2 已从官方 GitHub 仓库锁定 `a10b902` 并完成真实端到端验收，豆瓣目录和豆瓣想看当前随官方插件仓库锁定 `e5b5a4b`，真实 API、多 Provider Web Console、失败隔离、持久游标、独立启停和启动恢复均已验收。豆瓣目录关键词搜索使用移动端 `rexxar/api/v2/search`，因为旧 `/j/subject_suggest` 已在真实回归中出现 HTTP 200 空数组；插件必须只映射 `subjects.items` 中的电影和剧集，不能把图书或人物混入媒体目录。
