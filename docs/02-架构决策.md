...
Phase 10.3 在官方外部插件仓库逐个实现 TMDb、SeedHub、豆瓣目录和豆瓣想看等真实插件。插件开发期间必须以真实数据持续回归 Core；TMDb 端到端已于 2026-08-31 通过并冻结 Plugin API v2，SeedHub SOURCE v2 已从官方 GitHub 仓库锁定 `a10b902` 并完成真实端到端验收，豆瓣目录和豆瓣想看当前随官方插件仓库锁定 `e5b5a4b`，真实 API、多 Provider Web Console、失败隔离、持久游标、独立启停和启动恢复均已验收。Phase 10.4 的豆瓣候选 `94be9a8` 使用移动端 search、subject collection 和 recommend 作为无密钥主路径，搜索无有效影视项时降级解析豆瓣电影搜索页 `window.__DATA__`；插件过滤非影视卡片，Core 受控 HTTP 对 408、429 和 5xx 只做有限重试。不得复制成熟项目中用于 Frodo 私有协议的逆向密钥。
