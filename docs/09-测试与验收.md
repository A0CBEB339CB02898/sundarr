...
截至 2026-09-14，前端组件测试入口共 8 项通过，新增“清除筛选后忽略晚到筛选响应”的确定性竞态回归；Core 全量回归为 300 项通过、1 项需显式 PostgreSQL 条件的测试跳过，Web 生产构建和本轮 Ruff 检查通过。豆瓣插件默认回归为 33 项通过、5 项实时测试跳过，本轮目录显式实时回归 2 项通过。真实浏览器已验证豆瓣“剧情 -> 全部”恢复无筛选 URL 和热门电影。豆瓣目录以移动端公开 JSON 为主、网页搜索为降级；任何实时验收都不得把非官方接口的外部风控伪装为稳定官方 SLA。
