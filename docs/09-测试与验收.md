...
电影趋势、剧集趋势、分类推荐或关注更新单区失败时，其余分区仍可用。
URL query 不包含目录 Provider 凭据或其他敏感配置。
一级导航覆盖趋势、电影、剧集、动漫和综艺；动漫/综艺缺少 Provider 题材映射时显示明确不支持状态。
...
Phase 10.4 增加以下质量门：`alembic check` 不得报告未解释的模型漂移；前端至少具备可运行的组件测试入口；Python 使用 Ruff、类型检查和依赖漏洞检查；前端依赖不得使用无边界的 `latest` 作为版本合同。
