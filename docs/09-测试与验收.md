...
截至 2026-09-13，前端组件测试入口已经落地，2 个测试文件共 7 项通过；Core 全量回归为 299 项通过、1 项需显式 PostgreSQL 条件的测试跳过，Web 生产构建、Ruff 和 npm 高危漏洞审计通过。真实浏览器已验证 TMDb 热门、TMDb“情书”搜索、Provider 切换、豆瓣“情书”（1995）搜索和 URL 恢复，浏览器控制台无新增警告或错误。豆瓣分类公开接口仍可能因 IP 保护返回缺少 `data` 的业务响应，不把该外部波动记录为实时全绿。
