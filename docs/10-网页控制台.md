...
媒体发现 Core 与 Web Console 已完成结构性收口，不建设面向用户的 Mock 数据链；Phase 10.3 继续使用真实数据回归页面和 Core。TMDb 主 `CATALOG_PROVIDER` 已于 2026-08-31 通过真实海报墙、筛选搜索和详情页验收；豆瓣目录是待开发的可选补充 `CATALOG_PROVIDER`，豆瓣想看是待开发的独立 `WATCHLIST_PROVIDER`。数据采用核心身份与最小快照持久化、易变详情缓存的 A+ 策略。`/app/discover` 和详情路由已实现；未启用真实 Provider 时展示明确空状态。当前 `/app/search` 仍表示具体资源关键词聚合搜索。
...
不提供播放和观影进度。
不承担完整本地媒体管理。
已纳入当前 MVP 并完成通用 Core 与页面；TMDb 是已验收的真实主目录、豆瓣目录是当前开发的可选补充，豆瓣想看是独立列表接入。
