...
媒体发现 Core 与 Web Console 已完成结构性收口，不建设面向用户的 Mock 数据链；Phase 10.3 继续使用真实数据回归页面和 Core。TMDb 主 `CATALOG_PROVIDER` 与豆瓣补充 `CATALOG_PROVIDER` 已于 2026-08-31 通过真实海报墙、搜索、筛选、详情、来源切换和启动恢复验收；独立豆瓣想看 `WATCHLIST_PROVIDER` 已发布，当前验证真实条目在发现页关注更新区域的展示。数据采用核心身份与最小快照持久化、易变详情缓存的 A+ 策略。`/app/discover` 和详情路由已实现；未启用真实 Provider 时展示明确空状态。当前 `/app/search` 仍表示具体资源关键词聚合搜索。
