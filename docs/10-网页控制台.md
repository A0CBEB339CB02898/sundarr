...
当前没有独立 `/app/sync` 页面；同步绑定和扫描操作由远程媒体库页面承载。Phase 10.3 增加 `/app/plugins` 插件仓库管理入口，但不会提供 Python 代码编辑器。
...
说明：当前 Sources 页面是已安装搜索源的列表和测试入口。插件仓库管理使用独立 `/app/plugins`，属于 Phase 10.3，尚未实现。
