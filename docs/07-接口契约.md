...
`attribution` 包含 `provider_name`、`homepage_url`、`notice` 和可选 `logo_url`。这些值来自插件静态能力描述，不包含凭据或用户数据；Web Console 必须将其作为来源/致谢信息展示，不能把外部平台标识表现得比 Sundarr 主品牌更突出，也不能暗示平台背书。
