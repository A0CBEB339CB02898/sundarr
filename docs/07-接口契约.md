...
`GET /discover/providers` 返回每个 active Provider 的 `identity_namespaces`、operations、media_types、filters、sorts、`operation_filters`、`operation_sorts` 和 `filter_options`。`filters` / `sorts` 是 Provider 能力并集；`operation_filters` / `operation_sorts` 按 `search`、`trending`、`categories` 等操作给出实际可用子集，Core 校验和 Web Console 控件必须优先使用按操作声明。迁移期旧 Provider 没有对应操作项时回退到全局集合；明确声明空数组表示该操作不支持筛选或排序，不能回退。
