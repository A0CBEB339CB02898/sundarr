...
POST /transfers/{task_id}/retry-cleanup
GET  /transfers/{task_id}/logs
```

当前已实现上述接口。`retry` 只重新入队失败的传输任务；`retry-cleanup` 只接受主体传输已完成且错误码为 `SYNC_SOURCE_DELETE_FAILED` 或 `CLOUD_CLEANUP_FAILED` 的任务，只重试清理，不重新复制目标文件。
...
### 9.3 POST /plugins/repositories/{repo_id}/check

只 fetch 远端引用并比较 commit，不 checkout、不解析或执行候选插件，也不改变 `current_commit`。
...
### 9.4 PUT /plugins/repositories/{repo_id}

应用明确的候选 commit。Web Console 必须提交最近检查得到的 `new_commit`；候选继续经过 Manifest、配置、合同和健康检查，成功后原子切换，失败时保留旧版本。

### 9.5 POST /plugins/repositories/{repo_id}/rollback

回滚仓库到 previous_commit。

### 9.6 DELETE /plugins/repositories/{repo_id}

删除插件仓库及其关联插件配置。

### 9.7 GET /plugins/plugins
...
### 9.8 GET /plugins/plugins/{plugin_id}

获取单个插件详情。

### 9.9 GET /plugins/plugins/{plugin_id}/config

读取单个插件的脱敏配置。敏感字段统一返回 `***`。

### 9.10 PUT /plugins/plugins/{plugin_id}/config

更新插件配置。

### 9.11 POST /plugins/plugins/{plugin_id}/enable

启用插件。

### 9.12 POST /plugins/plugins/{plugin_id}/disable

禁用插件。

### 9.13 POST /plugins/plugins/{plugin_id}/test

对当前进程中 active 的插件执行一次显式健康检查。没有额外 `health_check` 的插件返回 Activation 正常，不伪造外部服务连通性。响应只包含脱敏消息、字符串详情和检查时间，不修改插件启停或锁定版本。

### 9.14 GET /plugins/stats
...
### 9.15 POST /plugins/load-all

加载所有已启用仓库的插件。

### 9.16 GET /configuration/readiness
...
ready 只表示是否存在 required 阻断项；存在 recommended 提醒时可以为 true。
插件必填项由 config_schema 的 required/default 与 PluginConfig 实际值判定。
同步链路按可执行的下一步渐进提示，避免一次展示所有后续依赖项。
至少一条已启用且来源、目标和 SMB 连接均可用的同步绑定即视为存在可执行链路。
未绑定目标的远程媒体库、未使用但最近测试失败的 SMB 连接返回 recommended，不得阻断其他有效链路。
```
...
`GET /discover/trending` 返回当前 Provider 自己定义的热度内容。官方 TMDb 插件使用 `/trending/movie/day` 或 `/trending/tv/day`；它与 TMDb 的 `/movie/popular`、`/tv/popular` 是不同数据集。Web Console 面向用户统一显示“热门电影 / 热门剧集”并展示来源，但不得声称结果与平台 `popular` 页面逐项一致，也不得进一步包装成热映、票房或评分榜。

Web Console 的“电影 / 剧集”一级入口使用 `GET /discover/categories`，在用户未指定排序时由 Provider 使用 `popularity` 默认排序；不得因为没有其他筛选条件就改走 `trending`。界面中的热门入口和热门排序虽然使用相同中文词，但分别对应 `trending` 内容流与 `popularity` 分类排序，调用路径必须保持明确。
...
`GET /discover/{media_subject_id}/poster?provider_id=...` 是外部海报直连失败后的受控同源中继。`provider_id` 必填，并且必须等于该 `MediaSubject.last_known_poster_source`；Core 从 PostgreSQL 读取 `last_known_poster_url`，不得接受 `url`、`referer` 或自定义请求头参数。`snapshot_source` 只表示最近一次最小快照写入来源，想看同步不得借此改变已有海报的中继 Provider。上游必须为 HTTPS、返回受支持的图片 MIME 且不超过 Core 大小上限；Core 拒绝本机/内网字面 IP、重定向、HTML 和其他非图片响应。成功响应设置 `X-Content-Type-Options: nosniff` 与短期浏览器缓存；主体不存在或没有海报返回 `404`，来源不匹配返回 `409`，Provider 不可用返回 `503`，上游图片读取失败返回 `502`。该接口不把海报二进制写入 PostgreSQL、Redis 或文件系统。
...
想看列表属于独立 `WATCHLIST_PROVIDER`，不绑定当前选中的目录 Provider。Web Console 打开想看条目详情时，只能传入能够识别该条目稳定外部 ID 的 active CATALOG_PROVIDER；当前目录 Provider 不兼容时应选择其他兼容 Provider，找不到时省略 `provider_id`，由 Core 返回可用的最小快照。不得把当前目录选择强制附加到不兼容的想看条目，也不得按标题猜测或静默创建跨平台绑定。
...
`poster_provider_id` 是 `poster_url` 的独立来源，用于构造受控海报中继地址；它可以与返回该条列表数据的 `provider_id` 不同，例如想看插件更新了条目状态，但海报仍来自豆瓣目录插件。
