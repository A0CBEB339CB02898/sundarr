...
`GET /discover/trending` 返回当前 Provider 自己定义的趋势榜单。官方 TMDb 插件使用 `/trending/movie/day` 或 `/trending/tv/day`；它与 TMDb 的 `/movie/popular`、`/tv/popular` 是不同数据集。Web Console 不得把该操作包装成能够与平台 `popular` 页面逐项对应的“热门榜”，应使用“电影趋势 / 剧集趋势”等准确文案并展示来源。

Web Console 的“电影 / 剧集”一级入口使用 `GET /discover/categories`，在用户未指定排序时由 Provider 使用 `popularity` 默认排序；不得因为没有其他筛选条件就改走 `trending`。这样“趋势”入口与“热门”排序分别对应两种明确的数据语义。
