...
`POST /discover/hydrate-years` 接受一个 `provider_id` 和 1 至 12 个 `media_subject_ids`。Core 只处理 PostgreSQL 中已存在且能被该 active Provider 识别的媒体主体；已有年份直接返回。缺失年份优先通过 Provider `search` 按规范标题查询，并仅接受稳定外部 ID 精确一致的结果；无法精确匹配时再通过 `detail` 合同补全并复用详情缓存。响应包含成功得到的 `years` 映射和仍未解决的 `unresolved_ids`；单条失败不把整批变成错误。Provider 不存在返回 `503`，Provider 同时未声明 `search` 和 `detail` 返回 `422`。该接口不得接受外部 ID、详情 URL或任意 Provider 私有参数。
