...
3. 下一步接入 v2 Activation、requires/provides、配置校验和健康检查。
...
通用 Manifest v2 多声明解析、requires/provides 静态校验、三类公共运行协议和类型专用 Runtime Registry 已实现；v2 入口 Activation、Registry 能力注入、健康检查、原子切换和启动加载待实现。
