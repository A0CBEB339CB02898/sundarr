...
关联：媒体库引用存储管理中的 SMB 连接。下载管理将远程媒体库同步到本地媒体库。

### 1.5 远程媒体库（网盘库）

职责：管理已挂载的远程目录（如网盘通过 SMB 暴露的目录）。

```text
远程媒体库是通过 SMB 访问的远程目录，通常是网盘挂载目录。
远程媒体库也绑定到某个 SMB 连接下的目录。
远程媒体库是下载的来源。
```

关联：远程媒体库引用存储管理中的 SMB 连接。下载管理将远程媒体库同步到本地媒体库。
...
```text
下载管理是系统的核心功能。
一个"同步绑定"连接：远程媒体库（来源） -> 本地媒体库（目标）。
Worker 定时扫描同步绑定，发现稳定文件后创建下载任务。
下载过程：从来源 SMB 读取 -> 写入 .downloading -> 校验 -> rename -> 按配置清理来源。
```

关联：下载管理引用远程媒体库和本地媒体库，通过 Worker 执行。
