...
媒体发现中心已进入当前 MVP 和当前优先任务。TMDb 是主 `CATALOG_PROVIDER`，豆瓣目录是可选补充 `CATALOG_PROVIDER`，豆瓣想看是独立 `WATCHLIST_PROVIDER`；数据采用核心身份与最小快照持久化、易变详情缓存的 A+ 策略，路由仍待后续决策。当前 `/app/search` 仍表示已实现的关键词聚合搜索基线，不得把发现中心写成已完成。
...
状态：当前已实现的关键词聚合搜索基线；当前 MVP 将其演进或并入媒体发现中心。
...
不提供播放和观影进度。
不承担完整本地媒体管理。
已纳入当前 MVP；TMDb 是主目录、豆瓣目录是可选补充，豆瓣想看是独立列表接入；A+ 持久化策略已确认，页面信息架构尚待确认。
