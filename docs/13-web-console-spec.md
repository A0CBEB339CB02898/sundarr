...
MVP 筛选栏只包含媒体类型、题材、地区、年份/年份范围和排序。
排序选项固定为热度、评分和上映时间。
题材和地区均使用单选控件；选择新值替换旧值，不形成标签集合。
不显示语言、评分区间、演员、导演、Provider 来源或复杂多选入口。
目录插件和想看插件在管理界面独立显示启用状态、测试结果和最后错误。
同一仓库由 Manifest v2 声明的多个插件按 plugin_id 分开展示和配置；仓库 commit 状态与插件 Activation 状态不得合并成一个模糊开关。
页面可以选择页码或“加载更多”，但不得把该交互写入插件 Manifest；Provider 分页只通过 Core continuation token 访问。
