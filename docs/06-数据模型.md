...
> 迁移兼容说明：Phase 9/9.5 之前的 `ingest_*`、`download_to_local_*` 表以及 Resource 旧展示字段不再参与 ORM 和业务读写。为避免升级时静默删除可能需要人工导出的历史数据，Alembic 明确把这些对象标记为只读遗留项；发布清理必须另做备份、导出和确认，不能由自动生成迁移直接删除。
...
最后可用海报的来源单独保存在 last_known_poster_source；想看同步等不提供海报的 Provider 不得改写它。
...
MVP 只建 movie / series 主体，不创建季、集层级；TMDb 真实链路验收不会扩大这一边界，季集模型等待后续产品范围确认。
...
snapshot_source 表示最近一次最小快照写入来源，不等同于海报来源；海报中继必须使用 last_known_poster_source，避免想看同步覆盖元数据来源后破坏图片防盗链处理。
```
...
auto_update 是历史兼容列，当前固定为 false；在没有安全调度、候选验证和失败通知设计前，不进入公共 API 或 Web Console。
```
...
config_data 存储插件配置载荷；非敏感字段可直接序列化，Manifest 标记为 secret/password 的字段必须在真实凭据验收前完成静态加密，不能以明文 JSON 落库。
...
静态加密主密钥不属于业务数据，不进入 PluginConfig 或其他数据库表，由部署环境在数据库外提供。
```
