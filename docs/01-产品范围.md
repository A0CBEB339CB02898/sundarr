...
发现首页采用内容探索式导航。顶部一级入口固定为“趋势 / 电影 / 剧集 / 动漫 / 综艺”，趋势位于首位；搜索框独立位于同一导航区域右侧。趋势入口展示电影趋势、剧集趋势、分类推荐和关注更新分区；趋势分区使用当前 Provider 的 `trending` 操作，不等同于平台另行定义的 `popular`、热映、票房或评分榜。其他入口展示对应内容列表。动漫和综艺是由当前 Provider 题材能力映射出的探索预设，不扩展公共 `MediaType` 枚举，也不得在 Provider 不支持时伪造结果。
