...
Phase 10.1 已完成仓库内多候选编排、原子切换和启动恢复。Phase 10.2 完成 Core 媒体发现通用消费合同；Phase 10.3 把官方外部仓库迁移到 v2 并逐个接入真实插件，以真实数据持续回归和修正 Core，首个 TMDb 端到端通过后冻结 Plugin API v2。迁移期继续读取 flat v1；新插件使用 v2。旧 flat v1 加载入口继续明确拒绝 v2，避免绕过候选校验流程。
