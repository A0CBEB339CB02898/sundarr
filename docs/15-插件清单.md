...
插件清单文件名为 `sundarr_plugin.toml`。本文档记录 flat v1 兼容格式和已经实现的通用 v2 格式。更新时间：2026-09-14。
...
Phase 10.1 至 Phase 10.4 已完成。Plugin API v2 保持冻结；`sundarr-sources` 中 SeedHub SOURCE v2 已从官方 GitHub 锁定 `a10b902` 并通过真实端到端验收；`sundarr-plugin` 的正式远程锁定为 `e5b5a4b`，豆瓣目录与豆瓣想看均已通过真实纵向验收，豆瓣目录稳定性加固本地候选为 `94be9a8`。迁移期继续读取 flat v1；新插件使用 v2。旧 flat v1 加载入口继续明确拒绝 v2，避免绕过候选校验流程。
