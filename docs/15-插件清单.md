...
`douban-catalog` 的公开目录访问不要求在 Manifest 中声明 Cookie。未来 `douban-watchlist` 仍是独立的 `WATCHLIST_PROVIDER` 声明；只有其真实接入方式和合规边界验证完成后，才定义用户标识或 Cookie 等配置字段，不能把想看列表凭据提前塞入目录插件。
