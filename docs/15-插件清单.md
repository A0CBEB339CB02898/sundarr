...
Phase 10.1 已完成仓库内多候选编排、原子切换和启动恢复，Phase 10.2 已完成 Core 媒体发现通用消费合同的结构性验收。Phase 10.3 的 TMDb 真实端到端已通过并冻结 Plugin API v2；`sundarr-sources` 中 SeedHub SOURCE v2 已从官方 GitHub 锁定 `a10b902` 并通过真实端到端验收；`sundarr-plugin` 中豆瓣目录已锁定 `3271203` 并通过真实端到端验收，下一项为豆瓣想看。迁移期继续读取 flat v1；新插件使用 v2。旧 flat v1 加载入口继续明确拒绝 v2，避免绕过候选校验流程。
