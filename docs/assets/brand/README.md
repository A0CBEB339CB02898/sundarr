...
|---|---|---|---|---|---|---|
| **hero**   | 64 px | 12 px  | 7 px   | 6 px   | 86%  | 文档封面、splash、关于页 |
| **brand**  | 16 px | 5 px   | 3.5 px | 2.5 px | 71%  | 顶栏、侧栏副标、导航品牌区 |
| **inline** | 14 px | 4.5 px | 3 px   | 2 px   | 67%  | 正文引用；< 13 px 退化为 `Sundarr` |

关于 **mr < ml** 的补偿：字母 `r` 的右上短臂让右 r 视觉上"漂"得更远，
所以右 margin 要小一点。但**补偿力度随字号变化**——小字号下短臂占字母
视觉权重 ~40%，大字号下只占 ~20%。所以 mr/ml 比例从 inline 的 67%
渐变到 hero 的 86%，到 about (96px) 完全 1:1 对称。

反直觉要点：**越小的字号，点要相对越大**——抗锯齿会吞掉小圆。但大字号
反过来，**点相对占比要变小**（hero 档 12/64 = 18.75%，about 档
14/96 ≈ 14.6%），否则点看起来像个"按钮"而不是一个"点"。
