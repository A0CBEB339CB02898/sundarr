...
- 顶部工具栏左侧使用可横向滚动的一级 Tab：“热门 / 电影 / 剧集 / 动漫 / 综艺”；右侧为独立搜索框。桌面保持单行，移动端拆为两行且 Tab 不换行。
- 筛选区使用标签行：题材多选，地区单选，年份使用年代预设并可内联展开精确年份，排序按 Provider 能力启用。标签至少具备默认、hover、focus-visible、selected、disabled 状态。
- 筛选区顶部显示“当前筛选”token，可逐项取消并一键清除；取消一级分类回到热门，取消题材只移除对应值。
- “更多筛选”使用页内 `<details>` 渐进展开，不使用弹窗或抽屉。未被 Core 和 Provider 支持的高级条件显示明确不可用状态，不制造可点击伪入口。
- 热门内容使用横向二级 Tab 展示热门电影、热门剧集、分类推荐和关注更新，一次只渲染一个子页；底层保持 `trending` 合同，并在分区说明中展示当前 Provider，不能暗示它等于平台 `popular`、热映、票房或评分榜。每个子 Tab 独立缓存列表、分页、加载与错误状态。其他一级入口、有效关键词或筛选状态使用统一响应式海报网格。
