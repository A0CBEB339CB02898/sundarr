...
Token names extend (not replace) the existing CSS variables in `web/src/styles/base.css` so the rewrite is additive. `web/src/styles.css` 只保留模块导入并固定级联顺序。Names marked `(new)` don't exist yet.
...
- 主导航固定为四项：发现、收藏、任务、设置。具体资源搜索作为发现页内模式；配置型与低频运维页面不再各占一个主导航项。
- 设置页使用可横向滚动的二级 Tab：媒体源、插件、存储、本地媒体库、远程媒体库、系统状态；保留旧 URL 并映射到对应 Tab。
...
- 页面提供“内容发现 / 资源搜索”模式切换。内容发现调用 CATALOG_PROVIDER，资源搜索调用 SOURCE；两种模式共享产品入口但不共享筛选和结果状态。
- 顶部工具栏左侧使用可横向滚动的一级 Tab：“趋势 / 电影 / 剧集 / 动漫 / 综艺”；右侧为独立搜索框。桌面保持单行，移动端拆为两行且 Tab 不换行。
- 筛选区使用标签行：题材多选，地区单选，年份使用年代预设并可内联展开精确年份，排序按 Provider 能力启用。标签至少具备默认、hover、focus-visible、selected、disabled 状态。
- 筛选区顶部显示“当前筛选”token，可逐项取消并一键清除；取消一级分类回到趋势，取消题材只移除对应值。
- “更多筛选”使用页内 `<details>` 渐进展开，不使用弹窗或抽屉。未被 Core 和 Provider 支持的高级条件显示明确不可用状态，不制造可点击伪入口。
- 趋势内容使用横向二级 Tab 展示电影趋势、剧集趋势、分类推荐和关注更新，一次只渲染一个子页；趋势名称必须与 `trending` 合同一致，并在分区说明中展示当前 Provider，不能用含混的“热门电影”暗示它等于平台 `popular` 榜。每个子 Tab 独立缓存列表、分页、加载与错误状态。其他一级入口、有效关键词或筛选状态使用统一响应式海报网格。
- Provider 切换使用紧凑平铺标签；数据来源超过可用宽度时允许横向滚动，不使用下拉框。
- 卡片与详情页复用同一 `MediaSubject` 展示组件；各分区拥有独立标题和 loading / empty / error 状态。
- 单个分区或 Provider 失败不得替换整个页面为全屏错误。
- 关注更新缺少海报时，对当前可见卡片做有界渐进补全；使用显式选中的目录 Provider 和稳定外部 ID，不按标题猜测。卡片在原位从“正在补全海报”过渡为真实海报或明确的“暂无海报”。
- 关注更新的详情入口不继承不兼容的当前目录 Provider。优先使用当前 Provider；无法识别条目时改用已启用且声明对应身份命名空间的 Provider；仍找不到时展示 Core 最小快照，不让整页因跨 Provider 绑定失败而关闭。

### 7.2.1 `/app/settings` — 设置

- 页面顶部使用单层横向 Tab，承载媒体源、插件、存储、本地媒体库、远程媒体库和系统状态。
- 子页面保留原业务组件与旧直达路由，嵌入设置页时不重复展示第二个页面级标题。
- 插件仓库和实例状态统一显示中文；底层协议枚举仍保持英文。

### 7.2.2 `/app/discover?mode=resources` — 资源搜索

> 当前蓝图对应已实现的具体资源关键词聚合搜索；它属于发现页内的独立业务模式，旧 `/app/search` 仅作兼容入口。
...
4. **Brand**: 资产已落位 `docs/assets/brand/`。Web Console 已通过
   ...
   独立、带版本标识的 SVG favicon 与 `theme-color`；favicon 必须复用 Primary Mark，避免浏览器长期缓存旧图标。后续如需安装到移动端主屏，再补 180 px
   `apple-touch-icon`。
