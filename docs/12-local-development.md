...
Windows 本地开发默认使用项目内 `.sundarr\pytest-tmp` 和 `.sundarr\pytest-cache`。不要依赖系统 Temp、pytest 内置 basetemp 或默认 `.pytest_cache`，也不要手动传入 `--basetemp`，避免混合权限运行后生成当前用户无法清理的私有 ACL。
