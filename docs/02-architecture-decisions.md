...
## ADR-013: TMDb 作为 MVP 主目录，豆瓣想看独立接入
...
```text
TMDb 是媒体发现中心 MVP 的主目录数据提供方，负责搜索、筛选、热门、分类、电影/剧集详情和海报。
豆瓣想看是可选的独立接入，只负责产生或同步用户关注条目。
豆瓣接入不可用时，不得阻断 TMDb 目录浏览、用户搜索或其他发现入口。
...
尚待下一项决策：TMDb 和豆瓣接入的插件能力契约、manifest `provides` 表达方式，以及 Phase 10.1 需要恢复的最小插件运行时范围。
