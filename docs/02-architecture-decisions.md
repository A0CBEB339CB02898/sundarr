...
已在 `sundarr.app.plugins.contracts` 落地 v1 最小运行合同：目录 Provider 支持能力描述、搜索、热门、分类和详情；想看 Provider 接受 Core 管理的不透明游标并返回下一游标。Phase 10.2 已基于该合同实现 `MediaSubject` 数据模型、Core 编排和 API 模型；合同仍需通过 Phase 10.3 真实 TMDb 数据回归后冻结为 Plugin API v2。

当前实现已覆盖精确外部 ID 身份归并、缓存时效和同步游标持久化；跨 Provider 字段级合并优先级仍需由真实数据验证后收口。
...
Phase 10.3 当前优先，在官方外部插件仓库逐个实现 TMDb、豆瓣目录、豆瓣想看、SeedHub 等真实插件。插件开发期间必须以真实数据持续回归 Core；首个 TMDb 端到端通过并修正合同问题后冻结 Plugin API v2，再集中扩展其他插件。
